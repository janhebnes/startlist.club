...
namespace FlightJournal.Web.Migrations.FlightContext
{
    internal sealed partial class Configuration
    {
        ...
        { 
            ...
            /// Seed data for
            /// <list type="bullet">
            /// <item><description>TrainingPrograms SPL-S (SPL, spilstart)</description></item>
            /// <item><description>TrainingLessons (A0 til B8)</description></item>
            /// <item><description>TrainingExercises (e.g exSPL_A1_1 briefing inden flyvning)</description></item>
            /// </list> 
            /// </summary>
            /// <param name="context"></param>
            public static void InitializeTrainingPrograms(Models.FlightContext context)
            {
...
" };
            ...
            public static void GenerateTrainingFlights(Plane pl2, Location location, Pilot student, Pilot instructor, StartType start, Models.FlightContext context)
            {
                var trainingFlights = context.Flights.AddRange(new List<Flight>
                {
                    new Flight
                    {
                        ...
                        Landing = DateTime.Now.AddHours(-2).AddMinutes(15),
                        Plane = pl2,
                ...
                var lesson = context.TrainingLessons.FirstOrDefault();
                var exercise = context.TrainingExercises.FirstOrDefault();
                var grading = context.Gradings.FirstOrDefault(f => !f.IsOk);
                var gradingOk = context.Gradings.FirstOrDefault(f=>f.IsOk);
